...
with st.container(border=True):
    ...
    if 'mode' not in st.session_state or st.session_state.mode == "create":
        ...
        if st.button("🔎 ค้นหาเมนู", use_container_width=True):
            if not budget == 'ไม่จำกัดงบ (ระดับ Mr Beast)':
                prompt = (f"ฉันต้องการซื้ออาหาร {category} รสชาติ {taste} ราคาประมาณ {budget} ใน {country} "
                          f"แนะนำ 3 ตัวเลือกเมนู {category} ที่มีขายใน {country} คั่นด้วย '🍽️ เมนูที่' ไม่ต้องเกริ่นนำ และบอกราคาของอาหารนั้นๆด้วย และบอกด้วยว่าหาซื้อได้ที่ร้านไหน ที่มีขายในประเทศ {country}")
            else:
                prompt = (f"ฉันต้องการซื้ออาหาร {category} รสชาติ {taste} ราคามากกว่า 10000 บาท (ระดับ Mrbeast ซื้อกิน) ใน {country} "
                          f"แนะนำ 3 ตัวเลือกเมนู {category} ที่มีขายใน {country} คั่นด้วย '🍽️ เมนูที่' ไม่ต้องเกริ่นนำ บอกด้วยว่าหาซื้อได้ที่ร้านไหน และบอกราคาของอาหารนั้นๆด้วย ")
